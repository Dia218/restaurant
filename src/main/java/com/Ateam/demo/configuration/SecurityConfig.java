...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .authorizeRequests()
                .antMatchers( "/login", "/singUp", "/login_failed", "/resources/**").permitAll() // 로그인 권한은 누구나, resources파일도 모든권한
                // USER, ADMIN 접근 허용 설정
                .antMatchers("/guest").hasAnyRole("USER","ADMIN") //모두 가능
                .antMatchers("/manager").hasRole("ADMIN") //관리자만 가능
                .and()
                .formLogin()
                .loginPage("/login")
                .loginProcessingUrl("/login_proc")
                .defaultSuccessUrl("/guest")//manager
